...
| **verify** | Score touched code against the rubric `/18`; any blocker fails. | **Intent-fit** against the approved proposal/spec: scope respected, decisions unchanged, acceptance met. Drift = blocker. | **Both gates** |
...
`drift-detected` or `blocked` halts the chain at that phase. A blocked quality gate loops back into the
smallest fix and re-checks. `drift-detected` returns to the human as a change request (below).
